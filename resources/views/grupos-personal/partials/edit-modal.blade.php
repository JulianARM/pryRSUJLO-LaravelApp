<div class="modal fade" id="editGrupoPersonalModal{{ $group->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            {!! Form::model($group, [
                'route' => ['grupos-personal.update', $group],
                'method' => 'PUT',
                'class' => 'js-ajax-form js-personnel-group-form',
                'data-validate-url' => route('grupos-personal.validate'),
            ]) !!}
                {!! Form::hidden('ignored_group_id', $group->id) !!}
                <div class="modal-header bg-primary">
                    <h5 class="modal-title"><i class="fas fa-users mr-1"></i> Editar Grupo de Personal</h5>
                    <button type="button" class="close text-white" data-dismiss="modal" aria-label="Cerrar">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    @include('grupos-personal.partials.form', ['group' => $group])
                </div>
